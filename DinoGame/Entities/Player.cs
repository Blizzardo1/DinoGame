...
public class Player : Entity {
    ...
    public Player(nint renderer, float scale) : base(renderer,
            Path.Combine("Assets", "Tilesets", "Heroes", "Hero 007", "Hero_007.png"),
            32, 32, scale: scale) {
        ResetEntity();
    }
    ...
    public override void Update(Event sdlEvent) {
        ...
        if (!_isGrounded && _isJumping && !_isMaxJump) {
            ...
            UpdatePosition (Position.X, Position.Y + _velocity);
        }
        ...
        if (!_isJumping && Position.Y + Position.H >= Program.GetFloorY(TileSet)) {
            UpdatePosition(Position.X, Program.GetFloorY(TileSet)); // Prevent going below the ground
            _velocity = 0f;
